...
class Settings(EnvLoadable, DictMixin):
    ...
    # Redis
    reids_url: str = Attr(default='', env="REDIS_URL")
    reids_max_connections: int = Attr(default='', env="REDIS_MAX_CONNECTIONS")
    reids_encoding: str = Attr(default="utf-8", env="REDIS_ENCODING")
    reids_decode_responses: bool = Attr(default=True, env="REDIS_DECODE_RESPONSES")

    reids_socket_connect_timeout: int = Attr(default=5, env="REDIS_SOCKET_CONNECT_TIMEOUT")
    reids_socket_timeout: int = Attr(default=5, env="REDIS_SOCKET_TIMEOUT")

    reids_health_check_interval: int = Attr(default=30, env="REDIS_HEALTH_CHECK_INTERVAL")
    reids_retry_on_timeout: bool = Attr(default=True, env="REDIS_RETRY_ON_TIMEOUT")

    reids_ssl: bool = Attr(default=False, env="REDIS_SSL")
    reids_ssl_cert_reqs: Optional[str] = Attr(default=None, env="REDIS_SSL_CERT_REQS")
    reids_ssl_ca_certs: Optional[str] = Attr(default=None, env="REDIS_SSL_CA_CERTS")
    reids_ssl_certfile: Optional[str] = Attr(default=None, env="REDIS_SSL_CERTFILE")
    reids_ssl_keyfile: Optional[str] = Attr(default=None, env="REDIS_SSL_KEYFILE")

    reids_serializer: str = Attr(default="json", env="REDIS_SERIALIZER")
    reids_compress: bool = Attr(default=False, env="REDIS_COMPRESS")
