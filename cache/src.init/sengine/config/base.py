...
class Settings(EnvLoadable, DictMixin):
    ...
    server_base_url: str = Attr(default="/", env="SERVER_BASE_PATH")
    server_static_path: str = Attr(default=None, env="SERVER_STATIC_PATH")
    server_config_file: str = Attr(default=None, env="SERVER_CONFIG_FILE")
    ...
    # Qdrant
    qdrant_host: str = Attr(default='localhost', env="QDRANT_HOST")
    qdrant_port: int = Attr(default=6333, env="QDRANT_PORT")
    qdrant_grpc_port: int = Attr(default=6334, env="QDRANT_GRPC_PORT")
    qdrant_api_key: Optional[str] = Attr(default=None, env="QDRANT_API_KEY")
    qdrant_timeout: int = Attr(default=120, env="QDRANT_TIMEOUT")
    qdrant_prefer_grpc: bool = Attr(default=True, env="QDRANT_PREFER_GRPC")

    # LLM
    llm_base_url: str = Attr(default='http://localhost:8000', env="LLM_BASE_URL")
    llm_default_model: str = Attr(default='', env="LLM_DEFAULT_MODEL")
    llm_api_key: str = Attr(default='dummy_key', env="LLM_API_KEY")
    llm_default_max_tokens: int = Attr(default=2048, env="LLM_DEFAULT_MAX_TOKENS")
    llm_default_temperature: float = Attr(default=0.1, env="LLM_DEFAULT_TEMPERATURE")
    llm_default_top_p: float = Attr(default=0.8, env="LLM_DEFAULT_TOP_P")

    # Embedding
    embedding_base_url: str = Attr(default='http://localhost:11434', env="EMBEDDING_BASE_URL")
    embedding_model_name: str = Attr(default='nomic-embed-text', env="EMBEDDING_MODEL_NAME")
    embedding_dim: int = Attr(default=768, env="EMBEDDING_DIM")
    embedding_timeout: int = Attr(default=300, env="EMBEDDING_TIMEOUT")
    embedding_max_concurrent: int = Attr(default=10, env="EMBEDDING_MAX_CONCURRENT")

    # Message service
    msg_base_url: str = Attr(default="", env="MSG_BASE_URL")
    msg_api_token: str = Attr(default="", env="MSG_API_TOKEN")
    msg_timeout: int = Attr(default=30, env="MSG_TIMEOUT")
