...
class AsyncMinioClient:
    ...
    def __init__(
        ...
        # Configure client parameters with timeout
        self.config = Config(
            region_name=region,
            s3={'addressing_style': 'path'},  # MinIO recommends path style
            read_timeout=300,  # 5 minutes timeout for read operations
...
class MinioManager:
    """
    Manager class for MinIO client initialization and lifecycle management.
    """

    def __init__(self):
        self._client: Optional[AsyncMinioClient] = None
    ...
    async def init(
        ...
        if self._client is not None:
            raise RuntimeError("MinioManager already initialized.")

        self._client = AsyncMinioClient(
            endpoint=endpoint,
        ...
        # try:
        #     res = await self._client.list_buckets()
        # except Exception as e:
        #     self._client = None
        #     raise RuntimeError(f"Failed to connect to MinIO: {e}")

    @property
    def client(self) -> AsyncMinioClient:
        """
        Get the MinIO client instance.

        :returns: AsyncMinioClient instance
        :raises RuntimeError: If manager not initialized
        """
        if self._client is None:
            raise RuntimeError("MinioManager not initialized. Call .init() first.")
        return self._client

    async def close(self) -> None:
        """Close the MinIO client manager."""
        # The client doesn't need explicit closing as it uses context managers
        self._client = None
...
def get_minio_manager() -> MinioManager:
    """
    Get the global MinioManager instance.

    :returns: Global MinioManager instance
    """
